...
FORGEJO_HOST="forgejo.nelsor.net"
FORGEJO_PORT="63179"
FORGEJO_USER="shuri"
...
if [ "$HAS_FORGEJO_CREDS" = true ]; then
    ...
    git config --global user.name "shuri"
    git config --global user.email "shuri@wiki"
    ...
    if [ "$REPO_CHECK" = "200" ]; then
        ...
        CREATE_RESULT=$(curl -s -w '\n%{http_code}' \
            ...
            -d "{
                ...
                \"description\": \"Wiki.js content — auto-synced from CT 507\",
                \"private\": true,
                \"auto_init\": true,
                \"default_branch\": \"main\"
            }")
...
echo ""
echo -e "${GREEN}═══════════════════════════════════════════════════${NC}"
echo -e "${GREEN}  CT 507 (wiki) — Installation Complete${NC}"
echo -e "${GREEN}═══════════════════════════════════════════════════${NC}"
echo ""
